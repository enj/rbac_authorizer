...
plans and nothing outward happens. See [Publication](#publication) for what is
...
tools/go.mod                    the nested shim, requiring the pinned engine release
tools/go.sum                    only when -engine-sum supplies verified checksums
...
running engine can be read off the `go.mod`.

`-engine-sum` is optional and takes a *file* holding verified `go.sum` content,
because a module checksum cannot be computed from a checkout. Without it,
`tools/go.sum` is not written and the manifest records a notice telling you to
run `go mod download github.com/enj/soapbox/tools` inside `tools/` once the
pinned release exists, or to supply the checksums. With it, the content is
checked line by line and must cover both the module and its `/go.mod` entry.
...
3. **No backfill.** `sync` publishes one release. Resuming from a state record
...
5. **One commit per replay.** The pipeline replays the release commit as the
   root of its own epoch. Multi-commit traversal, anchor bounding, merge
   shaping, parent dedup, and unchanged-tree collapse are implemented and tested
   but no pipeline drives them yet.
6. **The state record omits the release tag.** It records the consumer branch
   ...
   commit.
7. **No vanity page generation.** See [vanity.md](vanity.md).
8. **No repository creation.** The GitHub API client can read a repository,
